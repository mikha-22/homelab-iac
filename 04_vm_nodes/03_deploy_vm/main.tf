...
# Provision the master node using previous template
resource "proxmox_virtual_environment_vm" "master" {
 ...
 # Add disk configuration with proper size
 disk {
   datastore_id = "cluster-shared-nfs"
   interface    = "scsi0"
   size         = module.shared.vm_configs.k3s_master.disk
 }
...
resource "proxmox_virtual_environment_vm" "worker" {
 ...
 disk {
   datastore_id = "cluster-shared-nfs"
   interface    = "scsi0"
   size         = module.shared.vm_configs.k3s_worker.disk
 }
